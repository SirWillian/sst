...
__attribute__((noinline))
static int some_other_function(int a, int b) { return a - b; }
static int (*orig_some_other_function)(int, int);
static int some_other_hook(int a, int b) {
	return orig_some_other_function(a, b) + 5;
}
...
TEST("Multiple functions should be able to be inline hooked at once", 0) {
	...
	orig_some_other_function = hook_inline(&some_other_function, &some_other_hook);
	if (!orig_some_other_function) return false;

	return some_other_function(5, 5) == 5;
}
